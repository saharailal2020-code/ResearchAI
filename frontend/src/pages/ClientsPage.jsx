  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-5 py-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500">Manajemen Client</p>
              <h2 className="mt-1 text-lg font-semibold text-slate-950">Daftar Client</h2>
            </div>
        ...
        <div className="divide-y divide-slate-200">
          {isLoading ? (
            <div className="px-5 py-8 text-sm font-medium text-slate-500">Memuat data client...</div>
          ) : clients.length === 0 ? (
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    {[client.industry || 'Industri belum diisi', client.city].filter(Boolean).join(' / ')}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-medium uppercase text-slate-400">Tipe</p>
                  <p className="mt-1 text-sm font-medium text-slate-700">{clientTypeLabel(client.client_type)}</p>
                </div>
                <div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div>
          <p className="text-sm font-medium text-slate-500">Client Baru</p>
          <h2 className="mt-1 text-lg font-semibold text-slate-950">Tambah Client</h2>
        </div>
        ...
        <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
            <label className="block">
              <span className="text-sm font-medium text-slate-700">Industri</span>
              <input
                className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-slate-950"
            ...
            <label className="block">
              <span className="text-sm font-medium text-slate-700">Tipe Client</span>
              <select
                className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-slate-950"
              ...
              >
                ...
                <option value="active_client">Client Aktif</option>
          ...
          <div className="border-t border-slate-200 pt-4">
            <p className="text-sm font-semibold text-slate-950">Contact Person Utama</p>
          ...
          <button
            ...
            type="submit"
          >
            {isSaving ? 'Menyimpan client...' : 'Simpan Client'}
          </button>
        </form>
      </section>
    </div>
